...
from ucp_clarify.config.base_config import BaseConfig, BaseInitializationConfig
...
def initialize_flags(
  ...
  flags.DEFINE_float(
      'beta_dpo_alpha',
      config.training_config.beta_dpo_alpha
      if hasattr(config.training_config, 'beta_dpo_alpha')
      else 0.6,
      'Scaling factor a for beta-DPO dynamic beta.',
  )

  flags.DEFINE_float(
      'beta_dpo_filter_ratio',
      config.training_config.beta_dpo_filter_ratio
      if hasattr(config.training_config, 'beta_dpo_filter_ratio')
      else 0.2,
      'Drop ratio used by beta-guided filtering.',
  )

  flags.DEFINE_float(
      'beta_dpo_ema_gamma',
      config.training_config.beta_dpo_ema_gamma
      if hasattr(config.training_config, 'beta_dpo_ema_gamma')
      else 0.9,
      'EMA factor used to update running discrepancy moments.',
  )

  flags.DEFINE_float(
      'beta_dpo_min_beta',
      config.training_config.beta_dpo_min_beta
      if hasattr(config.training_config, 'beta_dpo_min_beta')
      else 1.0e-3,
      'Lower clamp for beta-DPO dynamic beta.',
  )

  flags.DEFINE_float(
      'beta_dpo_max_beta',
      config.training_config.beta_dpo_max_beta
      if hasattr(config.training_config, 'beta_dpo_max_beta')
      else 0.15,
      'Upper clamp for beta-DPO dynamic beta.',
  )

  flags.DEFINE_integer(
      'beta_dpo_warmup_steps',
      config.training_config.beta_dpo_warmup_steps
      if hasattr(config.training_config, 'beta_dpo_warmup_steps')
      else 0,
      'Number of warmup steps using fixed base_beta before dynamic adjustment.',
  )

  flags.DEFINE_float(
      'clarify_loss_weight',
      config.training_config.clarify_loss_weight
      if hasattr(config.training_config, 'clarify_loss_weight')
      else 1.0,
      'Loss multiplier for CLARIFY preference pairs to counteract class imbalance.',
  )
  ...
  _mp = config.training_config.mixed_precision
  if isinstance(_mp, str):
      _mp = _mp.lower() not in ('no', 'false', '0', '')
  flags.DEFINE_bool(
      'mixed_precision',
      _mp,
  ...
  if isinstance(_loss_type_default, list):
      _loss_type_default = _loss_type_default[0] if _loss_type_default else None
  flags.DEFINE_string(
      'loss_type',
      _loss_type_default,
