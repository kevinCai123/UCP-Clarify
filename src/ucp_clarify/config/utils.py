...
from ucp_clarify.config.data.data_config import DataConfig
from ucp_clarify.config.model.hf_model_config import HFModelConfig
from ucp_clarify.config.model.model_config import ModelConfig
from ucp_clarify.config.training.training_config import (
    ACTConfig,
    ACTInitializationConfig,
)
from ucp_clarify.utils.storage_utils import read_json

_CONFIG_DIR = Path(__file__).resolve().parent
_DEFAULT_CONFIG = str(_CONFIG_DIR / "demo_config.json")
_DEFAULT_SFT_CONFIG = str(_CONFIG_DIR / "sft_demo_config.json")
...
def get_training_config(flags):
  ...
  return ACTConfig(
      ...
      output_dir=flags.output_dir,
      beta=flags.beta,
      experiment_mode=flags.experiment_mode,
      beta_dpo_alpha=flags.beta_dpo_alpha,
      beta_dpo_filter_ratio=flags.beta_dpo_filter_ratio,
      beta_dpo_ema_gamma=flags.beta_dpo_ema_gamma,
      beta_dpo_min_beta=flags.beta_dpo_min_beta,
      beta_dpo_max_beta=flags.beta_dpo_max_beta,
      beta_dpo_warmup_steps=flags.beta_dpo_warmup_steps,
      clarify_loss_weight=flags.clarify_loss_weight,
      gradient_checkpointing=flags.policy_model_gradient_checkpointing,
      gradient_checkpointing_kwargs=gradient_checkpointing_kwargs,
...
def get_config_from_dict(config):
  ...
  training_dict = dict(config["training_config"])
  try:
    training_cfg = ACTConfig(**training_dict)
  except TypeError:
    known_fields = {f.name for f in dataclasses.fields(ACTConfig)}
    training_dict = {k: v for k, v in training_dict.items() if k in known_fields}
    training_cfg = ACTConfig(**training_dict)
  preference_model_cfg = ModelConfig(**config["preference_model_config"])
  data_cfg = DataConfig(**config["data_config"])
